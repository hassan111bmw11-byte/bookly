...
import {
  ...
  Upload, BarChart3, Bell, Lock, Search, PlayCircle, Pencil, ShieldCheck, Users, Download,
  Image as ImageIcon, Eye, LogOut, Mail, Phone, Trophy, Bot,
  MessageSquareText, CreditCard, KeyRound, FolderOpen, LibraryBig,
  ClipboardList, ScanLine, Unlock, LockKeyhole, UserCheck,
  Send, Filter, MoreHorizontal, Database, HardDrive,
...
const coursesSeed: Course[] = chapters.map((title, i) => ({
  id: i + 1,
  title,
...
const teacherTools: ToolItem[] = [
  { title: "إدارة الكورسات", Icon: LibraryBig, text: "إنشاء فصل، ترتيب Playlists، قفل/فتح حسب الدفع." },
  { title: "رفع فيديوهات", Icon: Video, text: "رفع فيديو، صورة غلاف، ملفات مرفقة، وتصنيف داخل Playlist." },
  { title: "بنك الأسئلة", Icon: Database, text: "أسئلة MCQ/مقالي، مستويات، وسحب عشوائي للامتحانات." },
  { title: "Bubble Sheets", Icon: ScanLine, text: "إدخال نماذج وتصحيح ورقي رقمي سريع." },
  { title: "إدارة الطلاب", Icon: Users, text: "حسابات، حضور، مدفوعات، فتح محاضرات، وتجميد حساب." },
  { title: "الواجبات", Icon: ClipboardList, text: "تسليمات، ملفات، درجات، وتعليقات للطالب." },
  { title: "الرسائل والواتساب", Icon: MessageSquareText, text: "إرسال تنبيهات فردية أو جماعية وربط واتساب." },
  { title: "التقارير", Icon: LineChart, text: "مشاهدات، حضور، درجات، أرباح، ومستوى كل فصل." },
  { title: "التعليقات", Icon: MessageCircle, text: "مراجعة تعليقات المحاضرات والرد عليها." },
  { title: "الإعدادات", Icon: Settings, text: "بيانات المعلم، أرقام التواصل، الصلاحيات، والمظهر." },
];
...
export default function App() {
  ...
  const navStudent: NavItem[] = [
    { id: "dashboard", label: "الرئيسية", Icon: LayoutDashboard },
    { id: "courses", label: "المحاضرات", Icon: BookOpen },
    { id: "tasks", label: "الواجبات", Icon: ClipboardCheck },
    { id: "exams", label: "الاختبارات", Icon: Trophy },
    { id: "files", label: "الملفات", Icon: FolderOpen },
    { id: "wallet", label: "المحفظة", Icon: Wallet },
    { id: "chat", label: "الدردشة", Icon: MessageSquareText },
    { id: "ai", label: "المساعد الذكي", Icon: Bot },
    { id: "profile", label: "البروفايل", Icon: User },
    { id: "settings", label: "الإعدادات", Icon: Settings },
  ];

  const navTeacher: NavItem[] = [
    { id: "dashboard", label: "الرئيسية", Icon: LayoutDashboard },
    { id: "courses", label: "الكورسات", Icon: LibraryBig },
    { id: "videos", label: "الفيديوهات", Icon: ListVideo },
    { id: "exams", label: "الامتحانات", Icon: Trophy },
    { id: "questions", label: "بنك الأسئلة", Icon: Database },
    { id: "bubbles", label: "Bubble Sheets", Icon: ScanLine },
    { id: "students", label: "الطلاب والحضور", Icon: Users },
    { id: "chat", label: "الرسائل", Icon: MessageSquareText },
    { id: "reports", label: "التقارير", Icon: BarChart3 },
    { id: "profile", label: "البروفايل", Icon: User },
    { id: "settings", label: "الإعدادات", Icon: Settings },
  ];

  const nav: NavItem[] = isTeacher ? navTeacher : navStudent;
...
function AuthScreen({
  ...
  return <main dir="rtl" className={cls(theme, "min-h-screen p-4 md:p-8 transition-colors overflow-hidden")}>
    ...
    <div className="relative mx-auto max-w-7xl grid lg:grid-cols-[1.2fr_.8fr] gap-6 items-stretch">
      <section className="rounded-[2.4rem] border border-white/10 bg-linear-to-br from-slate-900 via-slate-950 to-black p-8 shadow-2xl overflow-hidden relative min-h-155">
        ...
        <div className="relative z-10 h-full flex flex-col">
          ...
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {[
              { title: "فتح وقفل حسب الدفع", Icon: LockKeyhole },
              { title: "Realtime Chat", Icon: MessageSquareText },
              { title: "رفع حتى 1TB", Icon: HardDrive },
              { title: "Playlists للفيديو", Icon: ListVideo },
              { title: "تصحيح اختبارات", Icon: ClipboardCheck },
              { title: "تقارير ودرجات", Icon: BarChart3 },
            ].map(({ title, Icon }) => <div key={title} className="rounded-2xl bg-white/10 p-4 border border-white/10"><Icon className="text-yellow-400 mb-2"/>{title}</div>)}
          </div>
          <div className="mt-auto rounded-3xl bg-white/10 border border-white/10 p-5 flex flex-col md:flex-row gap-4 md:items-center justify-between">
            ...
            <div className="flex -space-x-3 space-x-reverse">{[1,2,3,4].map(i=><div key={i} className="h-12 w-12 rounded-full border-2 border-slate-900 bg-linear-to-br from-yellow-300 to-slate-600" />)}</div>
          </div>
        </div>
      </section>
...
function Topbar({ dark, setDark, isTeacher, teacherName, studentName, notice }: {
  dark: boolean;
  setDark: SetBool;
  isTeacher: boolean;
  teacherName: string;
  studentName: string;
  notice: string;
}){return <header className="flex flex-col xl:flex-row gap-4 xl:items-center justify-between">
...
function Dashboard({ isTeacher, wallet }: { isTeacher: boolean; wallet: number }){return <div className="mt-8 space-y-6">
  <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-4">{(isTeacher ? [
    { title: "إجمالي الطلاب", value: "1,248", Icon: Users },
    { title: "الفيديوهات", value: "186", Icon: Video },
    { title: "امتحانات نشطة", value: "42", Icon: Trophy },
    { title: "دخل الشهر", value: "68,400 ج.م", Icon: Wallet },
  ] : [
    { title: "رصيد المحفظة", value: `${wallet} ج.م`, Icon: Wallet },
    { title: "محاضرات مفتوحة", value: "28", Icon: Unlock },
    { title: "واجبات مطلوبة", value: "4", Icon: ClipboardCheck },
    { title: "اختبارات قادمة", value: "3", Icon: Trophy },
  ] as StatItem[]).map(({ title: a, value: b, Icon }, i) =><motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:i*.05}} key={a} className="rounded-3xl border dark:border-white/10 bg-white dark:bg-slate-900 p-5 shadow-sm"><div className="flex items-center justify-between"><Icon className="text-yellow-500"/><MoreHorizontal className="text-slate-400"/></div><p className="mt-4 text-slate-500">{a}</p><b className="text-3xl">{b}</b></motion.div>)}</div>
  {isTeacher ? <TeacherTools/> : <StudentHome/>}
</div>}

function Panel({ title, action, children }: { title: string; action: string; children: React.ReactNode }){return <div className="mt-8 rounded-4xl border dark:border-white/10 bg-white dark:bg-slate-900 p-5 shadow-sm"><div className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between mb-5"><h2 className="text-2xl font-black">{title}</h2><button className="rounded-2xl bg-yellow-500 text-slate-950 font-black px-4 py-3 flex items-center justify-center gap-2"><Plus size={18}/>{action}</button></div>{children}</div>}
function Card({ icon: Icon, title, text }: { icon: LucideIcon; title: string; text: string }){return <div className="rounded-3xl border dark:border-white/10 p-5 bg-slate-50 dark:bg-slate-950/70 hover:-translate-y-1 transition"><Icon className="text-yellow-500"/><h3 className="font-black mt-3">{title}</h3><p className="text-slate-500 mt-1 leading-7">{text}</p></div>}
